...
literal `--` before path arguments. Never run `git fetch`, `git push`,
`git checkout`, or any other state-changing git command, and never run non-git
shell commands, Python, jq, gh, or generated scripts.
