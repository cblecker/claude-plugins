...
arguments and single-quote every path — paths come from the untrusted diff
and may contain shell metacharacters. Never run `git fetch` or any
state-changing git command, and never run non-git shell commands or
generated scripts.
