...
function fixture(t, { fork = false, unrelatedHistory = false } = {}) {
  ...
  const calls = [];
  const options = { cwd: source, worktreeRoot: join(root, 'worktrees'), contextRoot: join(root, 'contexts'),
...
for (const change of ['head', 'base', 'closed']) test(`stops before allocation when PR ${change} changes`, t => {
  ...
  assert.ok(!existsSync(options.worktreeRoot));
});
...
test('missing pinned commit stops before allocation', t => {
  const { options, metadata } = fixture(t);
...
test('failed worktree add cleans only its empty allocations', t => {
  ...
  options.gitCommand = (cwd, ...args) => {
    if (args[0] === 'worktree' && args[1] === 'add') throw Error('add failed');
    return original(cwd, ...args);
  };
  assert.throws(() => prepare(url, options), /add failed/);
  assert.ok(!existsSync(options.worktreeRoot));
  assert.ok(!existsSync(options.contextRoot));
});
test('post-allocation failure removes its clean worktree and preserves other resources', t => {
  const { options, source } = fixture(t);
  mkdirSync(options.worktreeRoot); mkdirSync(options.contextRoot);
  writeFileSync(join(options.worktreeRoot, 'keep'), 'existing');
  const original = options.gitCommand;
  options.gitCommand = (cwd, ...args) => {
  ...
  assert.deepEqual(readdirSync(options.worktreeRoot), ['keep']);
  assert.deepEqual(readdirSync(options.contextRoot), []);
